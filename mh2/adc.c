...
#if CONFIG_MACH_MH21
    ADC_TEMPERATURE_PIN,
#elif CONFIG_MACH_MH22 || CONFIG_MACH_MH24x5
    ADC_TEMPERATURE_PIN, 0x00, 0x00,
#elif CONFIG_MACH_MH2401 || CONFIG_MACH_MH2446
...
// ADC timing:
// mh2103: ADC clock=4.5Mhz, Tconv=12.5, Tsamp=41.5, total=12.000us
// mh2407: ADC clock=21Mhz, Tconv=12, Tsamp=84, total=4.571us
// mh2446: ADC clock=22.5Mhz, Tconv=12, Tsamp=84, total=4.267us

// Perform calibration on mh2103
static void
...
gpio_adc_setup(uint32_t pin)
{
...
#if CONFIG_MACH_MH24x5 || CONFIG_MACH_MH2446
    if (chan >= 19) {
        // On the MH24, some ADC channels are only available from ADC3
        adc = ADC3;
        adc_base += 0x800;
        chan -= 19;
    }
    ...
    if (!is_enabled_pclock(adc_base)) {
        ...
        adc_calibrate(adc);
        uint32_t aticks = 4; // 4-12us sample time (depending on mh2 chip)
        adc->SMPR1 = (aticks | (aticks << 3) | (aticks << 6) | (aticks << 9)
                      | (aticks << 12) | (aticks << 15) | (aticks << 18)
                      | (aticks << 21)
                      | (CONFIG_MACH_MH24 ? aticks << 24 : 0));
