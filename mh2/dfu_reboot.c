// Reboot into MH2 ROM dfu bootloader
//
...
// Many MH2 chips have a USB capable "DFU bootloader" in their ROM.
...
dfu_reboot(void)
{
    if (!CONFIG_MH2_DFU_ROM_ADDRESS || !CONFIG_HAVE_BOOTLOADER_REQUEST)
        return;
    irq_disable();
...
dfu_reboot_check(void)
{
    ...
    *(uint64_t*)USB_BOOT_FLAG_ADDR = 0;
    uint32_t *sysbase = (uint32_t*)CONFIG_MH2_DFU_ROM_ADDRESS;
